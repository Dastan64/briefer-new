...
const TaskSubSection = ({ subsection, parentIndex, index }) => {
    const { subsectionTitle, tasks, id } = subsection;
    ...
    return (
        <section className="subsection">
            <h3 className="subsection__title"
                data-number={`${parentIndex}.${index + 1}`}>{subsectionTitle || 'Заголовок второго уровня'}
                {hours !== 0 && <span className="time">{hours} {declinate(hours, 'hours')}</span>}
            </h3>
            <p className="subsection__subtitle">Описание подраздела</p>
